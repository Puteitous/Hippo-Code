...
// 限制最大行数 5000，超限返回 null（不显示 diff）。
...
function computeChanges(origLines, curLines) {
  ...
  for (let d = 0; d <= max; d++) {
    ...
    for (let k = -d; k <= d; k += 2) {
      ...
      snap[idx] = x
      if (x >= N && y >= M) {
        return backtrackChanges(origLines, curLines, trace, snap, d, k, max)
      }
    }
    trace.push(snap)
  }
...
function backtrackChanges(origLines, curLines, trace, _lastSnap, lastD, lastK, max) {
